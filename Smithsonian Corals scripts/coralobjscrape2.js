...
const dir = path.join(__dirname, 'smithsonian_corals');
...
async function downloadObj(url) {
    ...
    const downloads = await page.evaluate(() => {
        const extractDetailsToJson = (document) => {
        ...
        document.querySelectorAll('.popover-body li a').forEach(item => {
            if (item.innerText.includes('obj') && item.innerText.toLowerCase().includes('low resolution')) {
                link = item.href;
            }
        });
        return {
    ...
    await downloadFile(downloads.link, `${dir}/${filename}`);
    return { title: downloads.title, file: filename, metadata: downloads.data };
}
...
async function main() {
    ...
    let metadata = {};
    try {
        const rawData = fs.readFileSync(`${dir}/metadata.json`, 'utf8');
        metadata = JSON.parse(rawData);
    } catch (error) {
        console.log('Metadata file does not exist. Starting with an empty object.');
    }

    const promises = links.slice(80, links.length).map(url => downloadObj(url).catch(console.error)); // Modify slice as needed
    const results = await Promise.all(promises);

    results.forEach(result => {
        if (result) {
            metadata[result.file] = { title: result.title, metadata: result.metadata };
        }
    });

    fs.writeFileSync(`${dir}/metadata.json`, JSON.stringify(metadata, null, 2));
    console.log(`All downloads completed and metadata (${Object.keys(metadata).length}) updated.`);
}
